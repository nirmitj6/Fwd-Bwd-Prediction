    ...
    def next_batch(self):
        ...
        if split=='train':
            if self.current_position + (B*T* self.num_processes+1) > len(self.tokens):
                self.current_position= self.B * self.T * self.process_rank
            buf =torch.flip(self.tokens[self.current_position : self.current_position+B*T+1], dims=[0])
...
if master_process:
    ...
    tokens=enc.encode("That is why I am sad.")
    tokens=torch.flip(torch.tensor(tokens,dtype=torch.long), dims=[0]) ### Flipped CHANGE ####
    ...
    for i in range(num_return_sequences):
        tokens=torch.flip(x[i,:max_length], dims=[0]).tolist() ## Flipped Change ##
